...
if (globalWithMongo._mongoClientPromise) {
  ...
  clientPromise = (async () => {
    // If local development environment detected, bypass ping check and load mock client immediately
    const isLocalFallback = MONGODB_URI.includes("localhost") || MONGODB_URI.includes("127.0.0.1");

    if (isLocalFallback) {
      console.warn(`Local development environment detected. Bypassing TCP ping check. Resolving MockMongoClient instantly.`);
      const mockClient = new MockMongoClient(MONGODB_URI);
      return mockClient.connect();
    }

    console.log(`Verifying MongoDB connection availability at ${MONGODB_URI}...`);
    const isRealAvailable = await testMongoConnection(MONGODB_URI);

    if (isRealAvailable) {
      console.log('Successfully pinged real MongoDB. Connecting with native mongodb driver...');
      const realClient = new MongoClient(MONGODB_URI);
      return realClient.connect();
    } else {
      console.warn(`Could not connect to MongoDB at ${MONGODB_URI}. FALLING BACK TO FILE MOCK DATABASE.`);
      const mockClient = new MockMongoClient(MONGODB_URI);
      return mockClient.connect();
    }
  })();
